...
class ParserModel(nn.Module):
    ...
    def forward(self, w):
        ...
        hidden = torch.matmul(x, self.embed_to_hidden_weight) + self.embed_to_hidden_bias # (batch_size, hidden_size)
        hidden = F.relu(hidden) # (batch_size, hidden_size)
        hidden = self.dropout(hidden) # (batch_size, hidden_size)
        logits = torch.matmul(hidden, self.hidden_to_logits_weight) + self.hidden_to_logits_bias # (batch_size, n_classes)
        ### END YOUR CODE
        return logits
