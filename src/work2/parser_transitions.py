...
def minibatch_parse(sentences, model, batch_size):
    ...
    while len(unfinished_parses) > 0:
        ...
        unfinished_parses = [pp for pp in unfinished_parses if len(pp.buffer) > 0 or len(pp.stack) > 1]  #将已经完成解析的引用对象从unfinished_parses中移除
    dependencies = [pp.dependencies for pp in partial_parses] #将partial_parses中每个对象的dependencies属性添加到dependencies列表中
    ### END YOUR CODE
